	...
	if test ${with_sqlite3:-default} = 'default' -a $supplied_sqlite3_tar_gz -gt 0; then
		...
		if test -f "$with_sqlite3/include/sqlite3.h"; then
			...
			if test ! -f config.status ; then
				...
				sqlite3_configure_options="--prefix=$with_sqlite3 --disable-amalgamation --disable-tcl --without-tcl --enable-threadsafe --enable-static --disable-shared"
...
AC_DEFUN(SNERT_INIT,[
	...
	AC_DEFINE_UNQUOTED($1_BUILD, $package_build)
